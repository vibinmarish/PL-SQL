...
/
CREATE TABLE STUDENT(NAME VARCHAR(10),EXAM MARKS);
INSERT INTO STUDENT VALUES('VIBIN',MARKS(98,100));
SELECT S.NAME,S.EXAM.ENGLISH,S.EXAM.JAVA FROM STUDENT S;

18--CREATE A NESTED TABLE AND INSERT VALUES
                                         
CREATE OR REPLACE TYPE GRADES AS OBJECT(
ENGLISH VARCHAR(5),
DS VARCHAR(5)
);
/

CREATE TYPE NESTED_TABLE AS TABLE OF GRADES;
/
CREATE TABLE SCHOOL(NAME VARCHAR(10),GPA NESTED_TABLE) NESTED TABLE GPA STORE AS TEMPS;
INSERT INTO SCHOOL VALUES('VIT',NESTED_TABLE(GRADES('A','A')));
